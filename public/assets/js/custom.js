...
$(document).ready(function () {
    ...
    $(".review-down").on("click", function () {
        $("html, body").animate(
            {
                scrollTop:
                    $(".product-tab-area .tabs #tab2").offset().top - 100,
            },
            "slow"
        );
        return false;
    ...
    $(".single-post-content .btn").on("click", function () {
        $("html, body").animate(
            { scrollTop: $(".post-comment-form").offset().top - 100 },
            "slow"
        );
        return false;
    });
    ...
    var $class = $("body");
    if (
        $class.hasClass("category") ||
        $class.hasClass("off-canvas-left") ||
        $class.hasClass("off-canvas-right")
    ) {
        $("#slider-range").slider({
            range: true,
            min: 55,
    ...
    $("#category-products .ttproduct-cat-item.even .tt_cat_content").each(
        function (index, element) {
            ...
            html +=
                ...
                $(element).find(".category-img").html() +
                "</div>";
            $(element).html(html);
        }
    );

    $(".layout5 #ttcmssubbanner .ttbannerblock .ttbanner:even").addClass(
        "even"
    );
    $(".layout5 #ttcmssubbanner .ttbannerblock .ttbanner:odd").addClass("odd");

    $(
        ".layout5 #ttcmssubbanner .ttbannerblock .ttbanner:even .ttbanner-content"
    ).each(function (index, element) {
        var html = "";
        html +=
            '<div class="ttbanner-inner right-block text-center">' +
            $(element).find(".ttbanner-inner").html() +
            "</div>";
        html +=
            '<div class="ttbanner-img left-block">' +
            $(element).find(".ttbanner-img").html() +
            "</div>";
        $(element).html(html);
    });
    ...
    /*--- start blog js ----*/
    $(
        ".layout1 .smartblog-content,.layout4 .smartblog-content,.layout5 .smartblog-content,.layout2 .smartblog-content"
    ).owlCarousel({
        items: 3, //1 items above 1000px browser width
        nav: false,
        navText: [
    ...
    $(".small-image-slider-single-product-tabstyle-3 a").on(
        "click",
        function (e) {
            e.preventDefault();
            ...
            $thisParent
                .find(".small-image-slider-single-product-tabstyle-3 a")
                .removeClass("active");
            $(this).addClass("active");

            $thisParent
                .find(".product-large-image .tab-pane")
                .removeClass("active show");
            $thisParent
                .find(".product-large-image " + $href)
  ...
  the text field element and an array of possible autocompleted values:*/
    ...
    var $grid = $(
        "#blog-masonary .products-grid .row,#blog-masonary-3 .products-grid .row"
    ).masonry({
        itemSelector: ".ttblog",
...
function responsivemenu(status) {
    if ($(document).width() <= 991) {
        $(".header-default .header-main .container .header-right").insertAfter(
            ".navbar-responsive"
        );
    } else {
        $(".header-default .menu .container .header-right").insertAfter(
            ".header-middle"
        );
    }
}
...
function responsiveheaderleft(status) {
    if ($(document).width() <= 991) {
        $(".header-default .header-left").insertAfter(
...
function fixed_productblock() {
    ...
    if (
        jQuery(
            "#product-sticky-right .product-information,#product-grid .product-information"
        ).height() >
        jQuery(
            "#product-sticky-right .left-column,#product-grid .left-column"
        ).height()
    )
        return;
